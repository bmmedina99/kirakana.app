...
function KanaCardComponent({
  ...
  return (
    <article
      className={`relative rounded-2xl border bg-mauve-50 p-4 shadow-sm transition duration-200 ${isActive ? `${theme.border} ${theme.softBackground}` : 'border-linen-150'}`}
    >
      <button
        type='button'
      ...
      >
        ...
        <span className='block mt-4 text-xs font-semibold tracking-widest uppercase text-copper-200'>
          {item.romaji}
        </span>
      ...
      <button
        ...
        className={`absolute right-3 top-3 grid size-9 place-items-center rounded-full transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-35 ${theme.focusRing} ${isSpeaking ? `${theme.softBackground} ${theme.text}` : 'bg-linen-50 text-copper-100 hover:bg-linen-100'}`}
      >
        <svg
          viewBox='0 0 24 24'
          fill='none'
